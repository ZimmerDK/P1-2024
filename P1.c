...
struct workout_result_t calculate_workout(struct set_data_t* data, struct workout_data_t* workout_data, int setCount) {
	...
	if (collectedScore > 9 + 0.0001) {
		if (workout_data->repCount == 8) {
			workoutResult.weightChange = -2.5;
			workoutResult.repChange = 4;
		}
		else if (workout_data->repCount > 8) {
			workoutResult.repChange = -1;
		}
	}
	else {
		if (maxScore != 10 && collectedScore < 8.5) {
			if (workout_data->repCount == 12) {
				workoutResult.weightChange = 2.5;
				workoutResult.repChange = -4;
			}
			else if (workout_data->repCount < 12) {
				workoutResult.repChange = 1;
			}
		}
	}
...
void calibrate_workout_routine(struct workout_data_t* calibration_data) {
...
int main(void) {
	...
	struct workout_data_t workoutData = { 10, 50.0 };

	setData[0].intensity = 7;
	setData[1].intensity = 8;
